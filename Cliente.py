    ...
    while running:
        try:
            ...
            if header == "MSG":
                ...
                # Procesar el tamaño de la imagen
                image_size = int(peer.recv(10).decode('utf-8'))
                alias = peer.recv(32).decode('utf-8').strip()  # Recibir alias del remitente
                image_data = b''
                received_size = 0

                # Recibir la imagen en bloques
                ...
                if received_size == image_size:
                    ...
                    # Mostrar la imagen con el alias
...
def send_message():
    ...
    if raw_message:
        ...
        # Agregar el encabezado "MSG" antes del mensaje
...
def send_image():
    try:
        ...
        # Enviar el encabezado, el tamaño de la imagen y el alias
        for peer in peers:
            try:
                peer.send("IMG".encode('utf-8'))
                peer.send(str(image_size).zfill(10).encode('utf-8'))
                peer.send(alias.encode('utf-8').ljust(32))  # Enviar alias con relleno a 32 bytes
                peer.sendall(image_data)
